<nav class="sticky top-0 z-50 bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl border-b border-gray-200 dark:border-gray-700">
        ...
        <div class="flex justify-between h-20">
            <div class="flex items-center space-x-10">
                <a href="{{ route('author.dashboard') }}"
                    ...
                    <span
                        class="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full ml-1 font-bold uppercase tracking-widest">Author</span>
                </a>

                <div class="hidden md:flex space-x-8 text-sm font-semibold uppercase tracking-wider">
                    <a href="{{ route('author.dashboard') }}"
                        class="hover:text-blue-600 transition-colors {{ request()->routeIs('author.dashboard') ? 'text-blue-600' : 'text-gray-500' }}">Dashboard</a>

                    <a href="{{ route('author.posts.index') }}"
                        class="hover:text-blue-600 transition-colors {{ request()->routeIs('author.posts.index') ? 'text-blue-600' : 'text-gray-500' }}">My
                        Posts</a>

                    <a href="{{ route('author.posts.create') }}"
                        class="hover:text-blue-600 transition-colors {{ request()->routeIs('author.posts.create') ? 'text-blue-600' : 'text-gray-500' }}">Write
                        New</a>
                </div>
            </div>
